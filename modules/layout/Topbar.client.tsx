...
export default function Topbar() {
  ...
  return (
    <header
      className={`border-border-surface bg-card flex flex-row justify-between border-b p-3 pr-5 delay-75 duration-300`}
    >
      <div className="xs:gap-4 flex items-center gap-2">
        {privatePage && (
          <Button icon={<PiAlignLeftDuotone />} onClick={toggleNavbar} />
        )}
        <Logo />
      </div>
